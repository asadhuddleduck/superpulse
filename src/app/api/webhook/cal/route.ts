    ...
    const upd = email
      ? await db.execute({
          sql: `UPDATE qualifier_responses
                SET demo_scheduled_at = ?, cal_booking_uid = ?, demo_booking_status = 'booked',
                    demo_offer_choice = 'yes',
                    demo_requested_at = COALESCE(demo_requested_at, ?),
                    updated_at = ?
                WHERE email = ?`,
          args: [startIso, uid, nowIso, nowIso, email],
        })
      : { rowsAffected: 0 };

    // Pull the richest contact details we have for the alert.
